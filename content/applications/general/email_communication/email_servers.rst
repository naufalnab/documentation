...
.. warning::
   ...
   :doc:`Configuring DNS Records for Email Sending <email_domain>`
...
On-Premise databases can use the :ref:`Command-line Interface (CLI)
<reference/cmdline/server/emails>` to add or edit outgoing email servers. This can be used to
create a transactional email server.  For SMTP accounts using :abbr:`2FA (Two Factor
Authentication)` an app password should be used. The server should also be set up in
:menuselection:`Settings --> Custom Mail Servers --> Outgoing Mail Servers --> New`. Then the
:guilabel:`Command Line Interface` option should be selected for the
:guilabel:`Authentication method`.

.. image:: email_servers/cli.png
   :align: center
   :alt: Command-line interface authentication method selected.
...
combination of the two system parameters: mail.default.from and mail.catchall.domain. For example,
“Admin” <admin@example.com> =>“Admin” <notifications@mycompany.com>.
...
.. tip::
   In SaaS and Odoo.SH Databases in order to utilize the pre-configured Odoo email server as a
   transactional email server to supplement the mass email server navigate to
   :menuselection:`Settings --> Technical --> Outgoing` and create a new outgoing email server. For
   :guilabel:`Authenticate with` select :guilabel:`Command-line Interface`. Test the connection and
   set a lower priority than the mass email server.
...
* **Reply messages** of messages sent from Odoo are routed to their original discussion thread (and
  ...
  alias (**catchall@**).
* **Bounced messages** are used as a Return-Path. This is especially
  used in `Odoo Email Marketing <https://www.odoo.com/page/email-marketing>`__. In this case bounces
  are opt-out based on if the email bounced to many times (5) in the last month and the bounces are
  at least, separated by one week. This is done to avoid blacklisting someone because of a
  ...
  Messages that bounce in the chatter will populate a red envelop indicating the failed delivery,
  including a reason.
...
be completed prior to going the database going live as if a customer replies after a change is
made then the system will not recognize the old alias and the reply won't be received.
...
Hosting a local email servers
-----------------------------

On-Premise databases can use a Local Server as an incoming email server. To do so create a new
incoming email server by navigating to :menuselection:`Settings --> Technical --> Incoming`.
Under :guilabel:`Server Type` select :guilabel:`Local Server` and under the
:guilabel:`Configuration` a script will populate to execute in the
:doc:`Command-line Interface <../../../developer/cli>`. This python script is unique to the
database the email server is being configured in.

.. image:: email_servers/local-server.png
   :align: center
   :alt: The Local Server is selected for the Server Type.
